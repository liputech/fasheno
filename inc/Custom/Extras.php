...
class Extras {
	...
	public function __construct() {
		...
		add_action( 'pre_get_posts', [ $this, 'fasheno_custom_pagesize' ],1 );
	...
	public function body_class( $classes ) {
		...
		$classes[] = 'fasheno-header-' . Opt::$header_style;
		$classes[] = 'fasheno-footer-' . Opt::$footer_style;
		...
		if ( fasheno_option( 'rt_tr_header_shadow' ) ) {
		...
		if ( fasheno_option( 'rt_sticky_header' ) ) {
			$classes[] = 'has-sticky-header';
		}

		if ( fasheno_option( 'rt_sticky_footer' ) ) {
			$classes[] = 'has-sticky-footer';
		}

		if ( fasheno_option( 'rt_blend' ) ) {
			$classes[] = 'rt-blend';
		}

		if ( fasheno_option( 'rt_remove_admin_bar' ) ) {
			$classes[] = 'remove-admin-bar';
		}

		if ( is_single() && Opt::$single_style ) {
			$classes[] = 'fasheno-single-' . Opt::$single_style;
		}
	...
	/*custom team archive */
	function fasheno_custom_pagesize( $query ) {
	...
	function menu_customize( $item_id, $item ) {
		// Mega menu
		$_mega_menu = get_post_meta( $item_id, 'fasheno_mega_menu', true );
		// Query string
		$menu_query_string = get_post_meta( $item_id, 'fasheno_menu_qs', true );
		?>

		<?php if ( $item->menu_item_parent < 1 ) : ?>
			<p class="description mega-menu-wrapper widefat">
				<label for="fasheno_mega_menu-<?php echo esc_attr($item_id); ?>" class="widefat">
					<?php _e( 'Make as Mega Menu', 'fasheno' ); ?><br>
					<select class="widefat" id="fasheno_mega_menu-<?php echo esc_attr($item_id); ?>" name="fasheno_mega_menu[<?php echo esc_attr($item_id); ?>]">
						<option value=""><?php _e( 'Choose Mega Menu', 'fasheno' ); ?></option>
						<?php
						for ( $item = 2; $item < 12; $item++ ) {
							...
							<option <?php echo esc_attr( $selected ); ?> value="<?php echo esc_attr( $class ); ?>">
								<?php printf( __( 'Mega menu - %1$s Col %2$s', 'fasheno' ), $menu_item, $label_hide ); ?>
							</option>
							<?php
						}
						?>
		...
		<p class="description widefat">
			<label class="widefat" for="fasheno-menu-qs-<?php echo esc_attr($item_id); ?>">
				<?php echo esc_html__( 'Query String', 'fasheno' ); ?><br>
				<input type="text"
					   class="widefat"
					   id="fasheno-menu-qs-<?php echo esc_attr($item_id); ?>"
					   name="fasheno-menu-qs[<?php echo esc_attr($item_id); ?>]"
	...
	function menu_update( $menu_id, $menu_item_db_id ) {
		$_mega_menu         = $_POST['fasheno_mega_menu'][ $menu_item_db_id ] ?? '';
		$query_string_value = $_POST['fasheno-menu-qs'][ $menu_item_db_id ] ?? '';

		update_post_meta( $menu_item_db_id, 'fasheno_mega_menu', $_mega_menu );
		update_post_meta( $menu_item_db_id, 'fasheno_menu_qs', $query_string_value );
	}
	...
	function menu_modify( $items, $menu, $args ) {
		foreach ( $items as $item ) {
			$menu_query_string = get_post_meta( $item->ID, 'fasheno_menu_qs', true );
	...
	public function search_form() {
		...
		<form method="get" class="fasheno-search-form" action="' . esc_url( home_url( '/' ) ) . '">
            <div class="search-box">
				<input type="text" class="form-control" placeholder="' . esc_attr__( 'Search here...', 'fasheno' ) . '" value="' . get_search_query() . '" name="s" />
				<button class="item-btn" type="submit">
					' . fasheno_get_svg( 'search', false ) . '
					<span class="btn-label">' . esc_html__( 'Search', 'fasheno' ) . '</span>
				</button>
            </div>
		</form>
		';
	...
	public function filter_language_attributes( $output, $doctype ) {
		...
		$color_mode = fasheno_option( 'code_mode_type' );
