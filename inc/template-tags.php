            ...
            <ul class="menu-icon-action <?php echo esc_attr( $menu_classes ) ?>">
				<?php if ( $args['search'] ) { ?>
                    <li class="rt-search-popup">
                        <a class="action-icon menu-search-bar rt-search-trigger" href="#header-search"
                           aria-label="search popup"><i class="icon-rt-search-2"></i></a>
                    </li>
				<?php }
				if ( $args['compare'] && class_exists( 'WooCommerce' ) && function_exists( 'rtsb' ) ) { ?>
				...
				<?php }
				if ( $args['wishlist'] && class_exists( 'WooCommerce' ) && function_exists( 'rtsb' ) ) { ?>
				...
				<?php }
				if ( $args['add_to_cart'] && class_exists( 'WooCommerce' ) && function_exists( 'rtsb' ) ) { ?>
				...
				if ( $args['login'] ) { ?>
                    <li class="rt-user-login rt-button">
                        <a class="action-icon" href="<?php echo esc_url( $args['login_link'] ) ?>"
...
if ( ! function_exists( 'fasheno_mobile_menu_icons_group' ) ) {
	...
	function fasheno_mobile_menu_icons_group( $args = [] ) {
		$default_args = [
			...
			'shop_page'    => fasheno_option( 'rt_mobile_header_shop_page' ),
		];
		$args         = wp_parse_args( $args, $default_args );
        ...
        <div class="mobile-action-button">
            <div class="menu-icon-wrapper">
                <ul class="menu-icon-action">
					...
					<?php }
					if ( $args['shop_page'] ) { ?>
						<li class="rt-shop-page">
							<a class="action-icon" href="<?php echo esc_url( $shop_page_url ) ?>"
							   aria-label="Shop Page">
								<i class="icon-rt-grid-two"></i>
							</a>
						</li>
					<?php }
					if ( $args['compare'] && class_exists( 'WooCommerce' ) && function_exists( 'rtsb' ) ) { ?>
...
if ( ! function_exists( 'fasheno_rtl_mode_icons' ) ) {
	/**
	 * Get menu color mode icon
	 * @return void
	 */
	function fasheno_rtl_mode_icons() {
		if ( fasheno_option( 'rt_rtl_mode' ) ) { ?>
            <div class="rtl-mode-switch">
				<?php
				if ( ! empty( $_GET['dir'] ) && $_GET['dir'] == 'rtl' ) {
					?>
                    <a class="fashion-rtl-btn" href="?dir=ltr">LTR</a>
					<?php
				} else {
					?>
                    <a class="fashion-rtl-btn" href="?dir=rtl">RTL</a>
					<?php
				}
				?>
            </div>
...
if ( ! function_exists( 'fasheno_product_ajax_search' ) ) {
	...
	function fasheno_product_ajax_search() {
		...
		if ( taxonomy_exists( 'product_cat' ) ) {
			$terms = get_terms( [ 'taxonomy' => 'product_cat', 'parent' => 0 ] );
			foreach ( $terms as $term ) {
				$category_dropdown[ $term->slug ] = [
					'name' => $term->name,
				];
			}
		}
                        ...
                        <li class="item rt-cat-drop cat-drop">
                            <div class="dropdown">
                                ...
                                <div class="cat-btn-wrap">
                                    <button class="rt-btn cat-toggle" type="button" id="dropdownMenuButton1"
                        ...
                        <li class="item rt-advanced-search flex-grow-1">
                            <div class="rt-input-group">
                                <input type="text" autocomplete="off" name="s"
                                       ...
                                       placeholder="<?php esc_attr_e( 'Type Your Products ...', 'fasheno' ); ?>"
                                       value="<?php echo esc_attr( $search ); ?>">
                                <div class="input-group-append">
                                    <input type="hidden" name="post_type" value="product">
                                    <button class="search-btn" aria-label="button"><i class="icon-rt-search-2"></i>
                                    </button>
                                </div>
                            </div>
                        </li>
...
if ( ! function_exists( 'fasheno_product_mobile_ajax_search' ) ) {
	/**
	 * product ajax search
	 * @return void
	 */

	function fasheno_product_mobile_ajax_search() {
		$category_dropdown = [];
		if ( taxonomy_exists( 'product_cat' ) ) {
			$terms = get_terms( [ 'taxonomy' => 'product_cat', 'parent' => 0 ] );
			foreach ( $terms as $term ) {
...
if ( ! function_exists( 'fasheno_site_logo' ) ) {
	...
	function fasheno_site_logo( $with_logo = false, $custom_title = '', $mode = '' ) {
		...
		<?php endif; ?>
        <a href="<?php echo esc_url( home_url( '/' ) ); ?>" rel="home" aria-label="site logo" class="site-logo">
			<?php
...
if ( ! class_exists( 'RT_POST_VIEWS' ) ) {
	...
	class RT_POST_VIEWS {
		function __construct() {
			add_action( 'wp_footer', [ $this, '_set_post_views' ] );
		}
	...
	function fasheno_post_thumbnail( $size = 'full', $thumb_date = false ) {
        ...
        <div class="post-thumbnail-wrap">
			...
			if ( ! empty( $rt_post_gallerys_raw ) && 'gallery' == get_post_format( get_the_ID() ) ) { ?>
                <div class="rt-swiper-slider single-post-slider rt-swiper-nav"
                     data-xld='<?php echo esc_attr( $swiper_data ); ?>'>
                    <div class="swiper-wrapper">
						<?php foreach ( $rt_post_gallerys as $rt_posts_gallery ) { ?>
                            <div class="swiper-slide">
								<?php echo wp_get_attachment_image( $rt_posts_gallery, $size, "", [ "class" => "img-responsive" ] );
								?>
                            </div>
						<?php } ?>
                    </div>
			...
			<?php } else { ?>
                <figure class="post-thumbnail">
                    <a class="post-thumb-link alignwide" href="<?php the_permalink(); ?>" aria-hidden="true"
                       tabindex="-1"><?php the_post_thumbnail( $size ); ?></a>
				...
				if ( fasheno_option( 'rt_video_visibility' ) == 1 && ( 'video' == get_post_format( get_the_ID() ) ) && ! empty( $rt_youtube_link ) ) { ?>
                    <div class="rt-video"><a class="popup-youtube video-popup-icon"
                                             href="<?php echo esc_url( $rt_youtube_link ); ?>"><i
                                    class="icon-rt-play-stroke"></i></a></div>
				<?php } ?>
			<?php } ?>
                ...
                <div class="profile-author">
                    ...
                    <div class="profile-content">
                        ...
                        <ul class="profile-author-social">
							<?php if ( ! empty( $prof_fb ) ) { ?>
                                <li><a href="<?php echo esc_attr( $prof_fb ); ?>"><i class="icon-rt-facebook-1"></i></a>
                                </li><?php } ?>
							<?php if ( ! empty( $prof_tw ) ) { ?>
                                <li><a href="<?php echo esc_attr( $prof_tw ); ?>"><i class="icon-rt-twitter-1"></i></a>
                                </li><?php } ?>
							<?php if ( ! empty( $prof_lk ) ) { ?>
                                <li><a href="<?php echo esc_attr( $prof_lk ); ?>"><i class="icon-rt-linkedin-1"></i></a>
                                </li><?php } ?>
							<?php if ( ! empty( $prof_pin ) ) { ?>
                                <li><a href="<?php echo esc_attr( $prof_pin ); ?>"><i
                                            class="icon-rt-pinterest-1"></i></a></li><?php } ?>
							<?php if ( ! empty( $prof_ins ) ) { ?>
                                <li><a href="<?php echo esc_attr( $prof_ins ); ?>"><i
                                            class="icon-rt-instagram-1"></i></a></li><?php } ?>
							<?php if ( ! empty( $prof_wht ) ) { ?>
                                <li><a href="<?php echo esc_attr( $prof_wht ); ?>"><i
                                            class="icon-rt-whatsapp-1"></i></a></li><?php } ?>
							<?php if ( ! empty( $prof_vim ) ) { ?>
                                <li><a href="<?php echo esc_attr( $prof_vim ); ?>"><i class="icon-rt-vine"></i></a>
                                </li><?php } ?>
							<?php if ( ! empty( $prof_you ) ) { ?>
                                <li><a href="<?php echo esc_attr( $prof_you ); ?>"><i class="icon-rt-youtube"></i></a>
                                </li><?php } ?>
                        </ul>
                    </div>
                </div>
